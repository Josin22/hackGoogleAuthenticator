...
/**
 * @author DX 这种方案建议只在开发调试的时候使用，因为这将损耗一些性能(需要额外加载apk文件)，调试没问题后，直接修改xposed_init文件为正确的类即可
 *     可以实现免重启，由于存在缓存，需要杀死宿主程序以后才能生效 Created by DX on 2017/10/4. Modified by chengxuncc on
 *     2019/4/16.
 */
public class HookLoader implements IXposedHookLoadPackage, IXposedHookZygoteInit {
  // 按照实际使用情况修改下面几项的值
  /** 当前Xposed模块的包名,方便寻找apk文件 */
  private static final String modulePackageName = BuildConfig.APPLICATION_ID;
    ...
    XposedHelpers.findAndHookMethod(
        ...
        new XC_MethodHook() {
          ...
          protected void afterHookedMethod(MethodHookParam param) throws Throwable {
            ...
            try {
              cls.getDeclaredMethod(initMethod, startupparam.getClass())
                  .invoke(instance, startupparam);
            } catch (NoSuchMethodException e) {
              // 找不到initZygote方法
            }
            cls.getDeclaredMethod(handleHookMethod, loadPackageParam.getClass())
  ...
  private Class<?> getApkClass(Context context, String modulePackageName, String handleHookClass)
      throws Throwable {
  ...
  private File findApkFile(Context context, String modulePackageName) {
    ...
    try {
      Context moudleContext =
          context.createPackageContext(
              modulePackageName, Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
